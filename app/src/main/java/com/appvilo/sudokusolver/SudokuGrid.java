...
public class SudokuGrid {
    ...
    public static EditText[][] gridCell;
...
//                space.setTextSize(0, 1);
                space.setBackgroundColor(Color.BLACK);
                if((a==0 || a==4 ||a==8 || a==12) && (b==0 || b==4 || b==8 || b==12)){
    ...
    private static EditText[][] initCell(Context context) {
        ...
        for (int i = 0; i < 9; i++) {
            for(int j=0; j<9; j++){
                ...
                Log.e("GNIDA"," "+cellValues[i][j]);
                sudokuCell[i][j] = new EditText(context);
                sudokuCell[i][j].setCursorVisible(false);
//                sudokuCell[i][j].setBackgroundColor(Color.LTGRAY);
                ...
                if(cellValues[i][j]!=0){
                    sudokuCell[i][j].setText(Integer.toString(cellValues[i][j]));
                    sudokuCell[i][j].setBackgroundResource(R.drawable.edit_text_style_get);
                } else {
    ...
    public static void getCellValues(){
        for(int i=0;i<9;i++){
            for(int j=0;j<9;j++){
                ...
                Log.e("GNIDA","Array - ["+i+"]["+j+"] = "+ cellValues[i][j]);
            }
        }
    }
